...
class Program:
  ...
  def display( self ):
    print( "pc: %d, acc: %d" % (self.pc, self.acc) )
  ...
  def execute( self ):
    ...
    if self.pc in self.call_stack:
      print("HALT")
      return -1

    self.call_stack.add( self.pc )
    inst = self.instructions[self.pc]
    (lhs, rhs) = inst.split(' ')

    ## only applies to part two
    if self.pc == self.inst_change:
      # swap the instruction to test and find the "corruption"
      if lhs == 'nop':
        lhs = 'jmp'
      elif lhs == 'jmp':
        lhs = 'nop'
  ...
  def modify_inst( self ):
    ...
    for i in range(self.inst_change+1, self.pc_max):
      inst = self.instructions[i]
      (lhs, rhs) = inst.split(' ')

      if lhs == 'nop' or lhs == 'jmp':
        # found one; store the index so when we see it we can flip it
        self.inst_change = i
        return
